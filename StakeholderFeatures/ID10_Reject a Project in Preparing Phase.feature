Feature: ID10_Reject a Project in Preparing Phase
    ...
    Scenario: (Normal Flow) Reject an existing Project in an existing Preparing phase which is not rejected by the Stakeholder
    ...
    Scenario: (Alternative Flow) Reject an existing Project in an existing Preparing Phase which is approved by the Stakeholder
        When the stakeholder chooses the list of the projects in Preparing phase with name
        <name>
